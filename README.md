...
下面的截图直接来自当前桌面端 `?demo=1` 内置演示状态，用于展示已经实现的页面与交互。任务进度、识别证据和硬件资源数字是固定 fixture，不代表当前电脑的实时跑分。

### 新建任务

在简约视图中选择视频来源、完整音画或仅音频范围，以及 Fast、Balanced、Accurate 处理档位。

![新建任务：视频来源、处理范围和三档质量配置](docs/assets/readme/create-task.webp)

### 处理工作台

数据工作室把阶段进度、当前执行引擎、实时画面、ASR/OCR 结果和统一证据时间轴放在同一个视图中。处理流程日志会持续记录阶段、消息、进度和指标；可以按阶段或警告/错误筛选、全文搜索、展开指标、复制或导出当前结果，用于定位任务停在了哪一步。

![简约视图：阶段进度、处理流程日志和证据轨](docs/assets/readme/processing-flow-guided.webp)

![数据工作室：更完整的硬件信息、阶段日志和专业检查区](docs/assets/readme/processing-flow-professional.webp)

### 笔记阅读与证据核查

完成后可以同时查看视频时间点、证据密度、语音与画面文字轨道，以及由同一份规范化文档生成的笔记。

![笔记阅读：视频、证据时间轴和规范化笔记](docs/assets/readme/note-reader.webp)

### 可选依赖与运行时

设置页会区分随程序携带、软件管理、本机发现和用户绑定的运行时，并展示能力、版本、体积、路径、升级与卸载操作。

![依赖与运行时：已发现实例、能力、版本、路径和管理操作](docs/assets/readme/runtime-manager.webp)

### 性能与资源配置

专业模式可以在自动建议的基础上调整 CPU/GPU 并发、视觉扫描、OCR、ASR 和验证预算；小白模式继续使用自动配置。

![性能配置：设备选择、并发和视觉识别参数](docs/assets/readme/performance-settings.webp)

### 模型协议与能力路由

供应商协议、认证、模型发现、能力声明和阶段路由分开管理，不兼容的模型不会出现在对应角色的候选列表中。

![模型设置：供应商协议、模型能力与处理角色路由](docs/assets/readme/model-protocols.webp)
...
- 在处理流程视图查看实时或历史阶段事件，筛选警告/错误并导出诊断日志；后端重启后，已经落盘的历史事件仍可读取。
...
任务页的“处理流程日志”把当前后端事件和该 run 已落盘的历史事件放在同一条时间顺序中。相同序号只显示一次，因此轮询实时进度时不会产生重复行。这里可以：

- 选择某个处理阶段，或只看警告、错误；
- 搜索阶段名、消息和指标；
- 展开单条事件的阶段指标，并在运行中自动跟随最新事件；
- 复制或导出当前筛选结果，交给开发者排障。

新任务会尝试把事件逐行追加到 `runs/<run-id>/logs/events.jsonl`。失败事件保留真正出错的阶段、异常类型和经过脱敏的错误摘要；日志文件暂时不可写不会使视频任务失败，已经生成的阶段产物也会保留。早期版本创建的 run 可能没有这个文件，此时界面显示空日志，而不是把任务误判为损坏。
...
├─ logs/events.jsonl         # 追加式处理事件，用于历史查看和失败诊断
